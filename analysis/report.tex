...
%%% what is it (what's the topology)? how does it work? explain why conjunctive neurons make it terribad, compare it to simulated annealing
...
First, the results on the RBM. We did not see the large scale-free jumps that we experienced in the Ising model and expected in the RBM. This is because the gradients with respect to the weights, which we were using as the fitness values, were local but didn't affect their neighbors in the proper self-organized criticality way, meaning that the co-evolutionary selection which we were implementing wasn't acting in a co-evolutionary way.
%%% it should be something which has a locality that's not weight-based, or which affects its neighbors. the critical dynamics depends upon that feature, which the gradient doesn't have.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Because of the fact that the majority of the time, $\tau$-extremal optimization is just local search, when it it possible to get to the global optimum by local search, it does so very quickly.
